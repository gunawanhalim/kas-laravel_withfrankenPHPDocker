(function($) {
	...
	$(function() {
		const maxValue = 100000000; // Set the maximum value for the progress circle
		function fetchTotalKas(startDate, endDate, search) {
			$.ajax({
				url: '/getTotalKas',
				method: 'GET',
				data: {
				...
				success: function(response) {
					const totalAmount = response.jumlahKas;
					const totalEntries = response.hitungKas;
					const pemasukan = response.pemasukan;
					const pengeluaran = response.pengeluaran;
					const totalPenjualan = response.totalPenjualan;
					const progressValue = totalAmount / maxValue;
					const progressPemasukan = pemasukan / maxValue;
					const progressPengeluaran = pengeluaran / maxValue;
					const progressPenjualan = totalPenjualan / maxValue;
	
					// Update the total amount display
					$('#total-kas').text('Rp. ' + totalAmount.toLocaleString('id-ID'));
					$('#pemasukan').text('Rp. ' + pemasukan.toLocaleString('id-ID'));
					$('#pengeluaran').text('Rp. ' + pengeluaran.toLocaleString('id-ID'));
					$('#totalPenjualan').text('Rp. ' + totalPenjualan.toLocaleString('id-ID'));
	
					// Update the total entries display
					$('#total-entries').text('Total Pemasukan dan Pengeluaran: ' + totalEntries);
	
					// Initialize and update the circle progress
					$('.harian-kas-1').circleProgress({
						value: progressValue,
						size: 125,
						thickness: 7,
						startAngle: 80,
				...
				error: function() {
					console.error('Failed to fetch total amount and total entries');
				}
			});
		}
	
		if ($(".harian-kas-1").length) {
			// Fetch initial data without filters
			fetchTotalKas();
	
			// Attach fetchTotalKas to filter button click event
			$('#filter-button').click(function() {
				var startDate = $('#startDate').val();
				var endDate = $('#endDate').val();
				var search = $('#searchInput').val();
				fetchTotalKas(startDate, endDate, search);
			});
		}
		...
		$(document).ready(function() {
			...
			var barChart; // Declare barChart variable to access later
		
			// Function to fetch and render data
			function fetchData(startDate, endDate, searchData) {
				$.ajax({
					url: '/kas-data',
					method: 'GET',
					data: {
					...
					success: function(response) {
						var labels = Object.keys(response.pemasukan);
						var pemasukanData = Object.values(response.pemasukan);
						var pengeluaranData = Object.values(response.pengeluaran);
			...
			// Attach fetchData to filter button click event
			...
			$('#download-pdf').click(function() {
				...
				var endDate = $('#endDate').val();
				var search = $('#searchData').val();
				window.open('/chart-pdf?start_date=' + startDate + '&end_date=' + endDate + '&search=' + search, '_blank');
			});
		});
		...
		if ($("#page-view-analytic").length) {
			$(function() {
				...
				function fetchData(startDate, endDate, search) {
					$.ajax({
						url: '/page-view-data',
						method: 'GET',
						data: {
						...
						success: function(response) {
							...
							var pageViewAnalyticOptions = {
								...
								legendCallback: function(chart) {
									...
									for (var i = 0; i < chart.data.datasets.length; i++) {
										text.push('<li><span class="legend-box" style="background:' + chart.data.datasets[i].borderColor + ';"></span><span class="legend-label text-dark">');
							...
							window.barChart = new Chart(barChartCanvas, {
								type: 'line',
								data: pageViewAnalyticData,
								options: pageViewAnalyticOptions,
							});
	
							document.getElementById('pageViewAnalyticLegend').innerHTML = barChart.generateLegend();
						},
						error: function() {
							console.error('Failed to fetch data from the server');
						}
				...
				$('#filter-analisis').on('click', function() {
					var startDate = $('#start-date').val();
					var endDate = $('#end-date').val();
					var search = $('#searchAnalisis').val();
					fetchData(startDate, endDate, search);
				});
			});
		}
		...
		// }
		// if ($(".harian-kas-1-dark").length) {
		// 	$('.harian-kas-1-dark').circleProgress({
		// 		value: 0.70,
		// 		size: 125,
		// 		thickness: 7,
		// 		startAngle: 10,
